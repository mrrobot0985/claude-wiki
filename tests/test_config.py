...
class TestLegacyRefusal:
    """ConfigManager.load() refuses legacy layout versions."""

    def test_load_refuses_legacy_layout_version_1(self):
        """A lock with layout_version '1' raises an actionable ConfigError."""
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir) / "my-project"
            repo.mkdir()
            marker = repo / ".claude-wiki.lock"
            ...
            manager = ConfigManager()
            with pytest.raises(
                ConfigError, match="Run 'claude-wiki migrate'"
            ) as exc_info:
                manager.load(repo)

            assert "Legacy layout version detected" in str(exc_info.value)

    def test_load_refuses_legacy_layout_version_none(self):
        """A lock with missing layout_version raises an actionable ConfigError."""
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir) / "my-project"
            repo.mkdir()
            marker = repo / ".claude-wiki.lock"
    ...
    def test_load_refuses_legacy_layout_version_empty(self):
        """A lock with empty layout_version raises an actionable ConfigError."""
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir) / "my-project"
            repo.mkdir()
            marker = repo / ".claude-wiki.lock"
    ...
    def test_load_succeeds_for_version_2(self):
        """A normal version-2 lock loads without error."""
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir) / "my-project"
            repo.mkdir()
            marker = repo / ".claude-wiki.lock"
    ...
    def test_load_succeeds_for_version_2_with_missing_field(self):
        """Defaults still apply for omitted fields on a version-2 lock."""
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir) / "my-project"
            repo.mkdir()
            marker = repo / ".claude-wiki.lock"
